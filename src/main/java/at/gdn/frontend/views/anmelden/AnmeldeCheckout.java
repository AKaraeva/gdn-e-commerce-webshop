...
import at.gdn.frontend.views.registrieren.RegistrierenCheckout;
...
@Route(value = "loginCheck", layout = MainLayout.class)
@Menu(order = 1)
@AnonymousAllowed
public class AnmeldeCheckout extends Composite<VerticalLayout> implements BeforeEnterObserver {
...
//        tabs.getStyle().set("padding", "20px");
        ...
        getContent().add(createGuestCheckoutButton());
        configureLayout();
    ...
    private Tabs createTabs() {
        ...
        tabs.addSelectedChangeListener(event -> {
            if (event.getSelectedTab() == anmeldenTab) {
                UI.getCurrent().navigate(at.gdn.frontend.views.anmelden.AnmeldeCheckout.class);
            } else if (event.getSelectedTab() == registrierenTab) {
                UI.getCurrent().navigate(RegistrierenCheckout.class);
            }
        });
    ...
    private Button createGuestCheckoutButton() {
        Button guestCheckoutButton = new Button("als Gast einkaufen!");
        guestCheckoutButton.addThemeVariants(ButtonVariant.LUMO_CONTRAST);
        guestCheckoutButton.getStyle()
                .set("margin-top", "-40px")
        ...
        guestCheckoutButton.addClickListener(event -> UI.getCurrent().navigate(CheckoutViewGuest.class));
