...
@Route(value = "/registrierenCheck", layout = MainLayout.class)
@Menu(order = 2, icon = LineAwesomeIconUrl.PENCIL_RULER_SOLID)
@AnonymousAllowed
public class RegistrierenCheckout extends Composite<VerticalLayout> {
    ...
    public RegistrierenCheckout(UserService userService) {
    ...
    private void configureLayout() {
        getContent().addClassName(LumoUtility.Padding.XLARGE);
    ...
    private Tabs createTabs() {
        ...
        tabs.addSelectedChangeListener(event -> {
            if (event.getSelectedTab() == registrierenTab) {
                UI.getCurrent().navigate(at.gdn.frontend.views.registrieren.RegistrierenCheckout.class);
            } else if (event.getSelectedTab() == anmeldenTab) {
                UI.getCurrent().navigate(AnmeldeCheckout.class);
            }
        });
